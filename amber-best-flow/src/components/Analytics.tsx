...
const Analytics = ({ userRole }: AnalyticsProps) => {
  ...
  const [yearlyCostSavingsViewMode, setYearlyCostSavingsViewMode] = useState<
    "yearly" | "currentMonth"
  >("yearly");
  const [yearlyCostSavingsFormat, setYearlyCostSavingsFormat] = useState<
    "lakhs" | "crores"
  >("lakhs");

  // Fetch plant performance data
  const { data: yearlyPlantPerformance, isLoading: isLoadingYearly } =
    usePlantPerformance("yearly", currentYear);
  const { data: monthlyPlantPerformance, isLoading: isLoadingMonthly } =
    usePlantPerformance("monthly", currentYear, currentMonth);

  // Fetch cost savings data
  const { data: yearlyCostSavingsData, isLoading: isLoadingYearlySavings } =
    useCostSavings("yearly", yearlyCostSavingsFormat, currentYear);
  const { data: monthlyCostSavingsData, isLoading: isLoadingMonthlySavings } =
    useCostSavings(
      "monthly",
      yearlyCostSavingsFormat,
      currentYear,
      currentMonth
    );
  ...
  // Show loading state
  if (
    (yearlyViewMode === "yearly" && isLoadingYearly) ||
    (yearlyViewMode === "currentMonth" && isLoadingMonthly)
  ) {
    return (
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold flex items-center">
              <BarChart3 className="h-6 w-6 mr-2 text-primary" /> Component
              Division Overview
            </h1>
            <p className="text-muted-foreground mt-1">
              {userRole === "plant"
                ? `${
                    user?.plant_name || user?.plant_short_name || "Plant"
                  } performance`
                : "Company-wide metrics and per-plant breakdown"}
            </p>
          </div>
    ...
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <Card className="shadow-soft hover:shadow-medium transition-smooth border border-border/50">
        ...
        <CardContent>
          ...
          (yearlyViewMode === "currentMonth" &&
            currentMonthPlantData.length === 0) ? (
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle className="flex items-center">
              ...
              {yearlyCostSavingsViewMode === "yearly"
                ? "Yearly Cost Savings"
        ...
        <CardContent>
          {(() => {
            const plantSavings =
              yearlyCostSavingsViewMode === "yearly"
                ? yearlyPlantSavings
                : currentMonthPlantSavings;
            const total = plantSavings.reduce((a, b) => a + b.savings, 0);

            // Show loading state
            if (
              (yearlyCostSavingsViewMode === "yearly" &&
                isLoadingYearlySavings) ||
              (yearlyCostSavingsViewMode === "currentMonth" &&
                isLoadingMonthlySavings)
            ) {
              return (
                <div className="text-center py-8 text-muted-foreground">
            ...
            return (
              <div className="space-y-4">
                <div className="flex items-center justify-between text-sm text-muted-foreground">
                  <span>
                    Plant-wise{" "}
                    {yearlyCostSavingsViewMode === "yearly" ? "YTD" : "Monthly"}{" "}
                    savings • ₹{" "}
                    {yearlyCostSavingsFormat === "crores" ? "Cr" : "L"}
                  </span>
                  <span className="font-medium text-foreground">
                    Company{" "}
                    {yearlyCostSavingsViewMode === "yearly" ? "YTD" : "Monthly"}
                    : {formatCurrency(total, 1, yearlyCostSavingsFormat)}
                  </span>
                </div>
                <ChartContainer
                  config={{
...
const generatePlantMonthlySavings = (
  practices: any[]
): Record<string, PlantMonthlyBreakdown[]> => {
  ...
  practicesByPlant.forEach((plantPractices, plantName) => {
    ...
    practicesByMonth.forEach((monthPractices, monthKey) => {
      ...
      const limitedPractices = monthPractices.slice(0, 2).map((p) => ({
        title: p.title,
        savings: extractSavingsAmount(
          p.savings_amount
            ? `₹${p.savings_amount}${
                p.savings_currency === "crores"
                  ? "Cr"
                  : p.savings_currency === "lakhs"
                  ? "L"
                  : ""
              }`
            : p.savings
      ...
      const totalSavings = limitedPractices.reduce(
        (sum, p) => sum + p.savings,
        0
      );
...
const getDefaultMonthlyBreakdown = (
  practices: any[]
): PlantMonthlyBreakdown[] => {
  if (practices.length === 0) return [];

  // Use the first practice as default
  const firstPractice = practices[0];
  const submittedDate =
    firstPractice.submitted_date || firstPractice.submittedDate;
  ...
  return [
    {
      month: monthKey,
      totalSavings: extractSavingsAmount(
        firstPractice.savings_amount
          ? `₹${firstPractice.savings_amount}${
              firstPractice.savings_currency === "crores"
                ? "Cr"
                : firstPractice.savings_currency === "lakhs"
                ? "L"
                : ""
            }`
          : firstPractice.savings
      ),
      practices: [
        {
...
const CostAnalysis = ({ userRole }: { userRole: "plant" | "hq" }) => {
  const navigate = useNavigate();
  const { user } = useAuth();
  const [costAnalysisFormat, setCostAnalysisFormat] = useState<
    "lakhs" | "crores"
  >("lakhs");

  // Fetch cost analysis data from API
  const { data: costAnalysisData, isLoading: isLoadingCostAnalysis } =
    useCostAnalysis(costAnalysisFormat);
  ...
  const plantMonthlySavings = useMemo(
    () => generatePlantMonthlySavings(practices),
    [practices]
  );
  const defaultMonthlyBreakdown = useMemo(
    () => getDefaultMonthlyBreakdown(practices),
    [practices]
  );
  ...
  const [selectedMonthPractices, setSelectedMonthPractices] = useState<{
    month: string;
    practices: { title: string; savings: number; benchmarked?: boolean }[];
  } | null>(null);

  // State for donut chart active indices (must be at top level for Rules of Hooks)
  const [currentChartActiveIndex, setCurrentChartActiveIndex] = useState<
    number | null
  >(null);
  const [lastChartActiveIndex, setLastChartActiveIndex] = useState<
    number | null
  >(null);
  const [yearlyChartActiveIndex, setYearlyChartActiveIndex] = useState<
    number | null
  >(null);
  ...
  const selectedPlantId = useMemo(() => {
    ...
    const plantData = costAnalysisData.data.find(
      (p) => p.plant_name === selectedPlant.name
    );
    return plantData?.plant_id;
  }, [selectedPlant, costAnalysisData]);

  // Fetch monthly breakdown for selected plant
  const { data: monthlyBreakdownData, isLoading: isLoadingMonthlyBreakdown } =
    usePlantMonthlyBreakdown(selectedPlantId, currentYear, costAnalysisFormat);

  const selectedPlantBreakdown = useMemo<PlantMonthlyBreakdown[]>(() => {
    const fillMissingMonths = (
      entries: PlantMonthlyBreakdown[]
    ): PlantMonthlyBreakdown[] => {
    ...
    if (!plantEntries || plantEntries.length === 0) {
      const plantPractice = practices.find(
        (p) => (p.plant_name || p.plant) === selectedPlant.name
      );
      if (plantPractice) {
        const submittedDate =
          plantPractice.submitted_date || plantPractice.submittedDate;
        const date = new Date(submittedDate || `${currentYear}-01-01`);
        const monthKey = `${date.getFullYear()}-${String(
          date.getMonth() + 1
        ).padStart(2, "0")}`;
        plantEntries = [
          {
            month: monthKey,
            totalSavings: extractSavingsAmount(
              plantPractice.savings_amount
                ? `₹${plantPractice.savings_amount}${
                    plantPractice.savings_currency === "crores"
                      ? "Cr"
                      : plantPractice.savings_currency === "lakhs"
                      ? "L"
                      : ""
                  }`
                : plantPractice.savings
            ),
            practices: [
              {
    ...
    <Card className="shadow-card">
      ...
      <CardContent className="space-y-6">
        ...
        {(() => {
          ...
          const plantsForCharts =
            userRole === "plant" && user?.plant_name
                  ...
                  >
                      ...
                      {data.map((entry, index) => {
                        return (
                          ...
                          >
                            <stop
                              offset="0%"
                              stopColor={entry.color.base}
                              stopOpacity={1}
                            />
                            <stop
                              offset="70%"
                              stopColor={entry.color.light}
                              stopOpacity={0.9}
                            />
                            <stop
                              offset="100%"
                              stopColor={entry.color.fade}
                              stopOpacity={0.4}
                            />
                          </linearGradient>
                        );
                    ...
                    <ChartTooltip
                      content={({ active, payload }) => {
                        if (active && payload && payload.length) {
                          const data = payload[0].payload;
                          const percent = ((data.value / total) * 100).toFixed(
                            1
                          );
                          return (
                      ...
                      label={({
                        ...
                        return (
                          <text
                            ...
                            style={{
                              textShadow:
                                "0 1px 3px rgba(255,255,255,0.95), 0 0 1px rgba(0,0,0,0.1)",
                            }}
                          >
            ...
            <div>
              <div
                className={`grid grid-cols-1 ${
                  showLastMonthChart ? "lg:grid-cols-3" : "lg:grid-cols-2"
                } gap-6`}
              >
                <div>
                  ...
                  {renderDonutChart(
                    ...
                    "Current Month",
                    "current",
                    currentChartActiveIndex,
                    setCurrentChartActiveIndex
                  )}
                </div>
                {showLastMonthChart && (
                  <div>
                    ...
                    {renderDonutChart(
                      ...
                      "Last Month",
                      "last",
                      lastChartActiveIndex,
                      setLastChartActiveIndex
                    )}
                  </div>
                )}
                <div>
                  ...
                  {renderDonutChart(
                    ...
                    "Yearly (YTD)",
                    "yearly",
                    yearlyChartActiveIndex,
                    setYearlyChartActiveIndex
                  )}
                </div>
              </div>
                  ...
                  <div className="bg-muted/30 rounded-lg p-5 border border-border/50">
                    <div className="flex flex-wrap justify-center items-center gap-4">
                      {uniquePlants.map((plant, index) => {
                        ...
                        const plantData = currentMonthData.find(
                          (p) => (p.fullName || p.name) === plant.name
                        );
                        const percent = plantData
                          ? (
                              (plantData.value / currentMonthTotal) *
                              100
                            ).toFixed(1)
                          : "0.0";
            ...
            <table className="w-full text-sm mt-4">
              ...
              <tbody className="divide-y">
                <tr className="hover:bg-accent/50">
                  ...
                  <td className="py-2">
                    {formatCurrency(
                      componentTotals.ytdTillLastMonth,
                      1,
                      costAnalysisFormat
                    )}
                  </td>
                ...
                <tr className="hover:bg-accent/50">
                  ...
                  <td className="py-2">
                    {formatCurrency(
                      companyTotals.ytdTillLastMonth,
                      1,
                      costAnalysisFormat
                    )}
                  </td>
            ...
            <tbody className="divide-y">
              {visible.map((p) => (
                ...
                >
                  ...
                  <td className="py-2">
                    {formatCurrency(p.lastMonth, 1, costAnalysisFormat)}
                  </td>
                  <td className="py-2">
                    {formatCurrency(p.currentMonth, 1, costAnalysisFormat)}
                  </td>
                  <td className="py-2">
                    {formatCurrency(p.ytdTillLastMonth, 1, costAnalysisFormat)}
                  </td>
                  <td className="py-2">
                    <Badge
                      variant="outline"
        ...
        <AlertDialog open={plantDetailOpen} onOpenChange={setPlantDetailOpen}>
          <AlertDialogContent className="max-w-3xl">
            <AlertDialogHeader>
              <AlertDialogTitle>
                {selectedPlant
                  ? `${selectedPlant.name} – Monthly Savings & BPs`
                  : "Plant Savings Details"}
              </AlertDialogTitle>
              <AlertDialogDescription>
                Breakdown of best practices uploaded each month and their
                contribution to savings (₹
                {costAnalysisFormat === "crores" ? "Cr" : "L"}).
              </AlertDialogDescription>
            </AlertDialogHeader>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                          ...
                          {hasPractices ? (
                            <div className="flex items-center justify-between gap-3">
                              <span className="text-sm font-medium">
                                {entry.practices.length}{" "}
                                {entry.practices.length === 1
                                  ? "Best Practice"
                                  : "Best Practices"}
                              </span>
                              <Button
        ...
        <AlertDialog
          open={practicesDialogOpen}
          onOpenChange={setPracticesDialogOpen}
        >
          <AlertDialogContent className="max-w-2xl">
            <AlertDialogHeader>
              <AlertDialogTitle>
                {selectedMonthPractices && selectedPlant
                  ? `${selectedPlant.name} – Best Practices for ${new Date(
                      selectedMonthPractices.month + "-01"
                    ).toLocaleDateString("en-IN", {
                      month: "long",
                      year: "numeric",
                    })}`
              ...
              <AlertDialogDescription>
                All best practices and their savings for this month (₹
                {costAnalysisFormat === "crores" ? "Cr" : "L"}).
              </AlertDialogDescription>
            </AlertDialogHeader>
            <div className="space-y-3 max-h-[60vh] overflow-y-auto">
              {selectedMonthPractices &&
              selectedMonthPractices.practices.length > 0 ? (
                selectedMonthPractices.practices.map((practice, idx) => {
                  // Find the full practice data from practices API data
                  const fullPractice =
                    practices.find(
                      (p) =>
                        p.title === practice.title &&
                        selectedPlant &&
                        (p.plant_name || p.plant) === selectedPlant.name
                    ) || practices.find((p) => p.title === practice.title);

                  // Format date - use practice submittedDate if available, otherwise use month
                  const submittedDate =
                    fullPractice?.submitted_date || fullPractice?.submittedDate;
